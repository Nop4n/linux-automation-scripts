#!/usr/bin/env python3
"""Gumroad uploader v4 - JSON body for products"""
...
def api_post(endpoint, fields=None, json_body=None):
    url = f"{API}/{endpoint}"
    if json_body:
...
def upload_file(filepath, token):
    ...
    etags = []
    for part in parts:
        req = urllib.request.Request(part["presigned_url"], data=file_data, method="PUT")
    ...
    # Step 3: Complete
    r = api_post("files/complete", json_body={
        "access_token": token,
        "upload_id": upload_id,
        "key": key,
        "parts": etags
...
def create_product(token, name, slug, price, desc, tags_list, file_url=None):
    ...
    if file_url:
        body["files"] = [{"url": file_url, "name": f"{slug}.zip"}]
    return api_post("products", json_body=body)
...
print("=" * 50)
print("Gumroad Product Uploader v4")
print("=" * 50)
print()
